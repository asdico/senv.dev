...
const config = {
  ...
  redirectUri: 'http://localhost:28785/functions/v1/connect-supabase/oauth2/callback',
}
...
router.get('/connect-supabase/oauth2/callback', async (ctx) => {
  ...
  if (projects){  
    for (const project of projects) {
      ...
      const response = await fetch(`https://api.supabase.com/v1/projects/${project.id}/branches`,{
        headers: {
      ...
      for (const branch of body){
        const secretsResponse = await fetch(`https://api.supabase.com/v1/projects/${branch.project_ref}/secrets`,{
          headers: {
            Authorization: `Bearer ${tokens.accessToken ?? tokens.access_token}`,
          },
        })
        const secretsBody = await secretsResponse.json()
        console.log('secretsBody****', secretsBody)
      }
...
const store = new CookieStore(Deno.env.get('SESSION_SECRET')!)
